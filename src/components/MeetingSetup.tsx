...
function MeetingSetup({ onSetupComplete }: { onSetupComplete: () => void }) {
  ...
  const [isJoining, setIsJoining] = useState(false); // Prevent multiple calls to handleJoin

  const call = useCall();

  if (!call) return null;

  // Enable/disable camera based on state
  useEffect(() => {
    if (isCameraDisabled) call.camera.disable();
    else call.camera.enable();
  }, [isCameraDisabled, call.camera]);

  // Enable/disable microphone based on state
  useEffect(() => {
